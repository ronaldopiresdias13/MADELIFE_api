...
class ChatGeralController extends Controller
{
    public function get_conversas()
    {
        ...
        $conversas = Conversa::whereHas('sender', function ($q) use ($profissinal) {
            $q->whereHas('profissional', function ($q2) use ($profissinal) {
                $q2->where('empresa_id', '=', $profissinal->empresa_id);
            });
        })->whereHas('receive', function ($q) use ($profissinal) {
            $q->whereHas('profissional', function ($q2) use ($profissinal) {
                $q2->where('empresa_id', '=', $profissinal->empresa_id);
            });
        })->select('conversas.*')->where(function ($q) use ($pessoa) {
