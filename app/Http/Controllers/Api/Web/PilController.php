...
class PilController extends Controller
{
    ...
    public function getPil(Request $request, $id)
    {
        ...
        // $cuidados = Cuidado::where('ativo','=',1)->where('empresa_id','=',$empresa_id)->orderBy('descricao')->get();
        $paciente_selecionado = Paciente::selectRaw('
        pacientes.id as paciente_id, pacientes.pessoa_id as pessoa_paciente_id,p.nome as paciente_nome, 
        pacientes.sexo as paciente_sexo, r.id as responsavel_id, pr.nome as responsavel_nome, r.parentesco,
        r.pessoa_id as pessoa_responsavel_id
        ')->where('pacientes.empresa_id', '=', $empresa_id)
            ->join(DB::raw('pessoas as p'), 'p.id', '=', 'pacientes.pessoa_id')
            ->join(DB::raw('responsaveis as r'), 'r.id', '=', 'pacientes.responsavel_id')
            ->join(DB::raw('pessoas as pr'), 'r.pessoa_id', '=', 'pr.id')->where('pacientes.id', '=', $pil->paciente_id)->first();
        ...
        $data_nead = Nead::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $pil->paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
        $data_abmid = PlanilhaAbmid::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $pil->paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
        $data_anexoa = PlanilhaAnexoA::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $pil->paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
    ...
    public function store_pil(PilRequest $request)
    {
        ...
        $nead_check = PlanilhaPil::where('empresa_id','=',$empresa_id)->where('paciente_id','=',$data['paciente_id'])->first();
        if($nead_check!=null ){
        ...
        $pil = new PlanilhaPil();
        $pil->fill([
            'diagnostico_primario_id' => $data['diagnostico_principal']['id'],
    ...
    public function get_related_paciente($type, $paciente_id)
    {
        ...
        $empresa_id = $user->pessoa->profissional->empresa_id;
        if ($type == 'pil') {
            $data = Nead::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            if ($data == null) {
                $data = PlanilhaAbmid::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            }
            if ($data == null) {
                $data = PlanilhaAnexoA::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            }

            $data_nead = Nead::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            $data_abmid = PlanilhaAbmid::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            $data_anexoa = PlanilhaAnexoA::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            ...
            $data = PlanilhaPil::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            if ($data == null) {
                $data = PlanilhaAbmid::where('empresa_id', '=', $empresa_id)->where('paciente_id', '=', $paciente_id)->with(['diagnostico_principal', 'diagnosticos_secundarios'])->first();
            }
            if ($data == null) {
